...
def list_credit_cards(db: Session = Depends(get_db)):
    return db.query(CreditCard).order_by(CreditCard.created_at.desc()).all()
...
def update_credit_card(card_id: str, payload: CreditCardUpdate, db: Session = Depends(get_db)):
    card = db.query(CreditCard).filter(CreditCard.id == card_id).first()
    if not card:
        raise HTTPException(status_code=404, detail="Credit card not found")
...
def list_card_installments(card_id: str, db: Session = Depends(get_db)):
