...
def seed_demo_data() -> None:
    ...
    try:
        ...
        if demo_user:
            ...
            db.query(Installment).filter(Installment.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(CreditCardPurchase).filter(
                CreditCardPurchase.user_id == demo_user.id
            ).delete(synchronize_session=False)
            db.query(Transaction).filter(Transaction.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(RecurringEntry).filter(
                RecurringEntry.user_id == demo_user.id
            ).delete(synchronize_session=False)
            db.query(CreditCard).filter(CreditCard.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(Budget).filter(Budget.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(SavingGoal).filter(SavingGoal.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(Investment).filter(Investment.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(Account).filter(Account.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.query(Category).filter(Category.user_id == demo_user.id).delete(
                synchronize_session=False
            )
            db.commit()
        ...
        for i in range(6):
            ...
            transactions.append(
                Transaction(
                    user_id=demo_user.id,
                    account_id=checking.id,
                    category_id=category_map["Housing"].id,
                    type="expense",
                    amount=1500.00,
            ...
            transactions.append(
                Transaction(
                    user_id=demo_user.id,
                    account_id=checking.id,
                    category_id=category_map["Utilities"].id,
                    type="expense",
                    amount=120.00,
                    description="Internet & Phone",
                    date=today.replace(day=10) - timedelta(days=30 * i),
                )
            )
            ...
            if random.random() > 0.3:
                transactions.append(
                    Transaction(
                        user_id=demo_user.id,
                        account_id=checking.id,
                        category_id=category_map["Dining Out"].id,
                        type="expense",
                        amount=round(random.uniform(4.5, 15.0), 2),
            ...
            if random.random() > 0.5:
                transactions.append(
                    Transaction(
                        ...
                        type="expense",
                        amount=round(random.uniform(15.0, 40.0), 2),
        ...
        print("Setting up recurring rules...")
        ...
        db.add_all(
            [
                RecurringEntry(
                    ...
                    description="TechCorp Salary",
                    category_id=category_map["Salary"].id,
                    category=category_map["Salary"].name,
                    frequency="monthly",
                ...
                RecurringEntry(
                    ...
                    description="Apartment Rent",
                    category_id=category_map["Housing"].id,
                    category=category_map["Housing"].name,
                    frequency="monthly",
                ...
                RecurringEntry(
                    ...
                    category_id=category_map["Entertainment"].id,
                    category=category_map["Entertainment"].name,
                    frequency="monthly",
                    start_date=next_month.replace(day=12),
                    active=True,
        ...
        def create_purchase(card, category, description, amount, installments, months_ago):
            ...
            purchase = CreditCardPurchase(
                ...
                first_installment_date=purchase_date + timedelta(days=20),
            ...
            for index in range(installments):
                due_date = purchase.first_installment_date + timedelta(days=30 * index)
                ...
                db.add(
                    Installment(
                        ...
                        is_paid=is_paid,
                        paid_account_id=checking.id if is_paid else None,
        ...
        create_purchase(mastercard, category_map["Transportation"], "Flights to Tokyo", 1200.00, 4, 3)
